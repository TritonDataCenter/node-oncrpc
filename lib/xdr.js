...
XDR.prototype.readRaw = function readRaw(nbytes) {
    assert.number(nbytes, 'num_bytes');

    var b = this.xdr_buffer.slice(this.xdr_offset, this.xdr_offset + nbytes);
    this.xdr_offset += nbytes;
...
XDR.prototype.writeDouble = function writeDouble(num) {
    ...
    // TODO: this is messed up - means we can only take 32b numbers for now
    this.writeInt(num);
    this.writeInt(0);
};
...
XDR.prototype.writeRaw = function writeRaw(buf) {
    ...
    for (var i = 0; i < buf.length; i++)
        this.xdr_buffer[this.xdr_offset++] = buf[i];
};
