...
PortmapClient.prototype.set = function set(opts, cb) {
    assert.object(opts, 'options');
    ...
    this._rpc(call, PortmapSetReply, function (err, reply) {
        if (err) {
            cb(err, reply);
            return;
        }
...
PortmapClient.prototype.unset = function unset(opts, cb) {
    assert.object(opts, 'options');
    assert.number(opts.prog, 'options.prog');
    assert.number(opts.vers, 'options.vers');
    assert.number(opts.prot, 'options.prot');
    ...
    this._rpc(call, PortmapUnsetReply, function (err, reply) {
        if (err) {
            cb(err, reply);
            return;
        }
